...
CONSUL_URL="${CONSUL_URL:-http://consul-test:8502}"
MAX_RETRIES=30
RETRY_INTERVAL=2
...
register_prometheus() {
    ...
    curl -X PUT --data '{
        ...
        "Check": {
            "HTTP": "http://prometheus-test:9092/-/healthy",
            "Interval": "10s",
            "Timeout": "5s"
        }
...
register_windows_exporter() {
    ...
    curl -X PUT --data '{
        ...
        "Check": {
            "HTTP": "http://windows-exporter-test:80/metrics",
            "Interval": "10s",
            "Timeout": "5s"
        }
    }' "$CONSUL_URL/v1/agent/service/register"
...
register_node_exporter() {
    ...
    curl -X PUT --data '{
        ...
        "Check": {
            "HTTP": "http://node-exporter-test:9100/metrics",
            "Interval": "10s",
            "Timeout": "5s"
        }
    }' "$CONSUL_URL/v1/agent/service/register"
